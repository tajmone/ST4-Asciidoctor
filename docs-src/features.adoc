...
For example, the "`Molokai`", "`Breakers`", and "`Celeste`" color schemes that ship with ST4, only highlight Heading level 1 differently from the others, while the AsciiDoc Dark scheme has a different color for each of the different levels (= Title, == Chapter, === Section, ==== Subsection, ...).
